...
public class Search {
	...
	/////////////////////////////////////////////////////////////////////////////////
public DoubleLinkedList searchContact(DoubleLinkedList contacts) {
		
		DoubleLinkedList contactsResult = new DoubleLinkedList();
		for(int i =0;i< contacts.size();i++) {
			
			Contact s = (Contact) contacts.get(i) ;
			
			//Search in Senders, Subjects, contents and dates
			String[] name = convertSentance(s.getUsername());
			String[] email = convertSentance(s.getEmail());
			

			if(BinarySearch(name,wordToSearch) != -1 
					||BinarySearch(email,wordToSearch) != -1 ) {
				contactsResult.add(contacts.get(i));
				continue;
			}
